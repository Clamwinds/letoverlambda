...
Cp.resolve = function () {
    each.call(arguments, function (resolver) {
        assert.strictEqual(typeof resolver === "undefined" ? "undefined" : _typeof(resolver), "function");
...
Cp.process = function (processor) {
    each.call(arguments, function (processor) {
        assert.strictEqual(typeof processor === "undefined" ? "undefined" : _typeof(processor), "function");
...
Cp.buildP = function (options, roots) {
    ...
    function rebuild() {
        ...
        if (self.preferredFileExtension) context.setPreferredFileExtension(self.preferredFileExtension);
    ...
    outputDir ? util.mkdirP : Q)(outputDir).then(rebuild);
};
...
function collectDepsP(rootModules) {
    ...
    function traverse(module) {
        if (seenIds.hasOwnProperty(module.id)) return Q(modules);
...
function cliBuildP(commoner, version) {
    ...
    if (argc === 0) {
        ...
        if (argc === 1) {
            ...
            roots = args.slice(2);
            if (roots.length === 0) roots.push(commoner.preferredFileExtension.glob());
        } else {
            options.help();
            process.exit(-1);
        }
...
function getConfigP(workingDir, configFile) {
    if (typeof configFile === "undefined") return Q({}); // Empty config.

    if (configFile === true || // --config is present but has no argument
    configFile === "<stdin>" || configFile === "-" || configFile === path.sep + path.join("dev", "stdin")) {
        return util.readJsonFromStdinP(1000, // Time limit in milliseconds before warning displayed.
        "Expecting configuration from STDIN (pass --config <file> " + "if stuck here)...", "yellow");
    }
...
//# sourceMappingURL=commoner-compiled.js.map