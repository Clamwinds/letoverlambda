...
function isReference(node) {
    ...
    return node.$refToScope || node.type === "Identifier" && !(parent.type === "VariableDeclarator" && parent.id === node) && // var|let|const $
...
function varify(ast, stats, allIdentifiers, changes) {
    ...
    ast.$scope.traverse({ pre: function pre(scope) {
            delete scope.moves;
        } });
}
...
function detectLoopClosures(ast) {
    ...
    function detectIifyBodyBlockers(body, node) {
        return breakable(function (brk) {
            traverse(body, { pre: function pre(n) {
...
function transformLoopClosures(root, ops, options) {
    ...
    traverse(root, { pre: function pre(node) {
            if (!node.$iify) {
                return;
            }
            ...
            var insertHead = hasBlock ? node.body.range[0] + 1 : // just after body {
            node.body.range[0]; // just before existing expression
            var insertFoot = hasBlock ? node.body.range[1] - 1 : // just before body }
...
function detectConstAssignment(ast) {
    traverse(ast, { pre: function pre(node) {
            if (isLvalue(node)) {
                var scope = node.$scope.lookup(node.name);
...
function setupScopeAndReferences(root, opts) {
    ...
    topScope.traverse({ pre: function pre(scope) {
...
//# sourceMappingURL=defs-main-compiled.js.map