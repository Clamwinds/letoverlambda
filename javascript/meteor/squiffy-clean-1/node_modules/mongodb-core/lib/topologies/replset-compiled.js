...
var executeWriteOperation = function executeWriteOperation(self, op, ns, ops, options, callback) {
  ...
  // Handler
  var handler = function handler(err, r) {
...
ReplSet.prototype.auth = function (mechanism, db) {
  ...
  if (this.s.authProviders[mechanism] == null && mechanism != 'default') throw new MongoError(f("auth provider %s does not exist", mechanism));
 ...
 * @method
 */
var handleIsmaster = function handleIsmaster(self) {
...
ReplSet.prototype.connect = function (_options) {
  ...
  // Additional options
  if (_options) for (var name in _options) {
    this.s.options[name] = _options[name];
  ...
  while (this.s.disconnectedServers.length > 0) {
    ...
    var execute = function execute(_server) {
      // Attempt to connect
...
ReplSet.prototype.isConnected = function (options) {
  ...
  if (options.readPreference && options.readPreference.equals(ReadPreference.secondary)) return this.s.replState.isSecondaryConnected();

  if (options.readPreference && options.readPreference.equals(ReadPreference.primary)) return this.s.replState.isSecondaryConnected() || this.s.replState.isPrimaryConnected();

  if (options.readPreference && options.readPreference.equals(ReadPreference.primaryPreferred)) return this.s.replState.isSecondaryConnected() || this.s.replState.isPrimaryConnected();

  if (options.readPreference && options.readPreference.equals(ReadPreference.secondaryPreferred)) return this.s.replState.isSecondaryConnected() || this.s.replState.isPrimaryConnected();

  if (this.s.secondaryOnlyConnectionAllowed && this.s.replState.isSecondaryConnected()) return true;
...
var addCredentials = function addCredentials(s, db, argsWithoutCallback) {
...
var clearCredentials = function clearCredentials(s, ns) {
  var db = ns.split('.')[0];
...
var pickServer = function pickServer(self, s, readPreference) {
  ...
  if (readPreference.equals(ReadPreference.primary) && s.replState.primary == null) throw new MongoError("no primary server available");
...
var setHaTimer = function setHaTimer(self, state) {
...
var haveAvailableServers = function haveAvailableServers(state) {
...
var replicasetInquirer = function replicasetInquirer(self, state, norepeat) {
  return function () {
    ...
    var timeoutServer = function timeoutServer(_server) {
    ...
    var inspectServer = function inspectServer(server) {
      if (state.replState.state == DESTROYED) return;
      // Did we get a server
    ...
    if (state.replState.primary != null && self.lastIsMaster() && Array.isArray(self.lastIsMaster().hosts) && !state.all) {
...
var errorHandlerTemp = function errorHandlerTemp(self, state, event) {
  return function (err, server) {
    ...
    if (state.initialConnectionServers == 0 && state.replState.state == CONNECTING) {
      if (state.secondaryOnlyConnectionAllowed && !state.replState.isSecondaryConnected() && !state.replState.isPrimaryConnected() || !state.secondaryOnlyConnectionAllowed && !state.replState.isPrimaryConnected()) {
        ...
        if (self.listeners('error').length > 0) return self.emit('error', new MongoError('no valid seed servers in list'));
      }
    }
    ...
    if (state.disconnectedServers.length == state.seedlist.length && state.replState.state == CONNECTING) {
      if (state.emitError && self.listeners('error').length > 0) {
        ...
        if (self.listeners('error').length > 0) self.emit('error', new MongoError('no valid seed servers in list'));
      }
    }
  };
};
...
// Connect handler
var connectHandler = function connectHandler(self, state) {
  return function (server) {
    ...
    var processNewServer = function processNewServer() {
      ...
      if (state.initialConnectionServers.length == 0 && Object.keys(state.connectingServers).length == 0 && !state.replState.isPrimaryConnected() && !state.secondaryOnlyConnectionAllowed && state.replState.state == CONNECTING) {
...
var processHosts = function processHosts(self, state, hosts) {
  if (state.replState.state == DESTROYED) return;
...
var inInitialConnectingServers = function inInitialConnectingServers(self, state, address) {
...
var connectToServer = function connectToServer(self, state, host, port, options) {
  options = options || {};
  var opts = cloneOptions(state.options);
  opts.host = host;
  opts.port = port;
...
var addToListIfNotExist = function addToListIfNotExist(list, server) {
  var found = false;
...
var errorHandler = function errorHandler(self, state) {
  return function (err, server) {
    if (state.replState.state == DESTROYED) return;
...
var timeoutHandler = function timeoutHandler(self, state) {
  return function (err, server) {
    if (state.replState.state == DESTROYED) return;
...
var closeHandler = function closeHandler(self, state) {
  return function (err, server) {
    if (state.replState.state == DESTROYED) return;
...
var notMasterError = function notMasterError(r) {
  // Get result of any
...
//# sourceMappingURL=replset-compiled.js.map