...
var mapReduce = function mapReduce(map, reduce, options, callback) {
  ...
  if (readPreference != false && readPreference != 'primary' && options['out'] && options['out'].inline != 1 && options['out'] != 'inline') {
  ...
  this.db._executeQueryCommand(cmd, { readPreference: readPreference }, function (err, result) {
    ...
    if (!result || !result.documents || result.documents.length == 0) return callback(Error("command failed to return results"), null);
...
var aggregate = function (pipeline, options, callback) {
  ...
  options = opts.readPreference || opts.explain || opts.cursor || opts.out || opts.maxTimeMS || opts.allowDiskUse ? args.pop() : {};
...
//# sourceMappingURL=aggregation-compiled.js.map