...
Options.prototype.init = function () {
  ...
  if (this.strategy != null && this.strategy != 'ping' && this.strategy != 'statistical' && this.strategy != 'none') throw new Error("Only ping or statistical strategies allowed");
  ...
  if (readPreference != null) {
    if (readPreference != ReadPreference.PRIMARY && readPreference != ReadPreference.PRIMARY_PREFERRED && readPreference != ReadPreference.SECONDARY && readPreference != ReadPreference.SECONDARY_PREFERRED && readPreference != ReadPreference.NEAREST && typeof readPreference != 'object' && readPreference['_type'] != 'ReadPreference') {
  ...
  if (this.readSecondary != null) this.readSecondary = this.readPreference == ReadPreference.PRIMARY || this.readPreference == false || this.readPreference == null ? false : true;

  // Ensure correct slave set
  if (this.readSecondary) this.slaveOk = true;

  // Set up logger if any set
  this.logger = this.logger != null && typeof this.logger.debug == 'function' && typeof this.logger.error == 'function' && typeof this.logger.debug == 'function' ? this.logger : { error: function (message, object) {}, log: function (message, object) {}, debug: function (message, object) {} };

  // Connection timeout
  this.connectTimeoutMS = typeof this.socketOptions.connectTimeoutMS == 'number' ? this.socketOptions.connectTimeoutMS : 1000;

  // Socket connection timeout
  this.socketTimeoutMS = typeof this.socketOptions.socketTimeoutMS == 'number' ? this.socketOptions.socketTimeoutMS : 30000;
};
