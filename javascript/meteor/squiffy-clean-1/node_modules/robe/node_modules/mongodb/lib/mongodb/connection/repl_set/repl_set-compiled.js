...
ReplSet.prototype.canRead = function (read) {
  if ((read == ReadPreference.PRIMARY || typeof read == 'object' && read.mode == ReadPreference.PRIMARY || read == null || read == false) && (this._state.master == null || !this._state.master.isConnected())) return false;
...
ReplSet.prototype.setSocketOptions = function (options) {
  ...
  if (typeof options.connectTimeoutMS == 'number') this.options.socketOptions.connectTimeoutMS = options.connectTimeoutMS;
...
ReplSet.prototype.connect = function (parent, options, callback) {
  if (this._serverState != ReplSet.REPLSET_DISCONNECTED) return callback(new Error("in process of connection"));

  // If no callback throw
  if (!(typeof callback == 'function')) throw new Error("cannot call ReplSet.prototype.connect with no callback function");
  ...
  this.once("fullsetup", function (err, db, replset) {
    ...
    // Emit fullsetup
    processor(function () {
      if (self.emitOpen) self._emitAcrossAllDbInstances(self, null, "open", null, null, null);
...
var _handler = function (event, self, server) {
  return function (err, doc) {
    ...
    // set to disconnected
    if (Object.keys(self._state.addresses).length == 0 && self._serverState != ReplSet.REPLSET_DESTROYED) {
...
var locateNewServers = function (self, state, candidateServers, ismaster) {
  ...
  for (var i = 0; i < hosts.length; i++) {
    if (!state.contains(hosts[i]) && !inCandidateServers(hosts[i], candidateServers)) {
...
ReplSet.prototype.isConnected = function (read) {
  ...
  if (read == null || read == ReadPreference.PRIMARY || read == false) isConnected = this._state.master != null && this._state.master.isConnected();

  if ((read == ReadPreference.PRIMARY_PREFERRED || read == ReadPreference.SECONDARY_PREFERRED || read == ReadPreference.NEAREST) && (this._state.master != null && this._state.master.isConnected() || this._state && this._state.secondaries && Object.keys(this._state.secondaries).length > 0)) {
    isConnected = true;
...
function _roundRobin(replset, tags) {
  ...
  var conn = null != replset._state.secondaries[key] ? replset._state.secondaries[key].checkoutReader() : null;
...
//# sourceMappingURL=repl_set-compiled.js.map