...
PingStrategy.prototype.checkoutConnection = function (tags, secondaryCandidates) {
  ...
  if (0 === finalCandidates.length) return new Error("No replica set members available for query");
  ...
  if (self.logger && self.logger.debug) {
    if (connection) self.logger.debug(format("picked server %s:%s", connection.socketOptions.host, connection.socketOptions.port));
  }
    ...
    for (var i = 0; i < allServers.length; i++) {
      ...
      new (function (serverInstance) {
        ...
        if (_db != null) {
          ...
          var connectTimeoutMS = self.replicaset.options.socketOptions ? self.replicaset.options.socketOptions.connectTimeoutMS : 0;
...
//# sourceMappingURL=ping_strategy-compiled.js.map