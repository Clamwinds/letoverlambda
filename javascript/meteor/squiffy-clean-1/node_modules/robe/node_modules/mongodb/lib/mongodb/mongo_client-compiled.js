...
MongoClient.connect = function (url, options, callback) {
  ...
  for (var i = 0; i < object.servers.length; i++) {
    ...
    var _server = object.servers[i].domain_socket ? new Server(object.servers[i].domain_socket, _server_options) : new Server(object.servers[i].host, object.servers[i].port, _server_options);
      ...
      new Db(object.dbName, __server, { w: 1, native_parser: false }).open(function (err, db) {
        ...
        if (totalNumberOfServers == 0) {
          ...
          if (totalNumberOfMongodServers == 0 && object.servers.length == 1) {
            ...
            serverConfig = obj.domain_socket ? new Server(obj.domain_socket, object.server_options) : new Server(obj.host, obj.port, object.server_options);
          } else if (totalNumberOfMongodServers > 0 || totalNumberOfMongosServers > 0) {
            var finalServers = object.servers.filter(function (serverObj) {
...
var _finishConnecting = function (serverConfig, object, options, callback) {
  ...
  if (object.db_options.read_preference) {
    ...
    if (object.db_options.read_preference_tags) readPreference = new ReadPreference(object.db_options.read_preference, object.db_options.read_preference_tags);
  ...
  db.open(function (err, db) {
    ...
    if (db.options !== null && !db.options.safe && !db.options.journal && !db.options.w && !db.options.fsync && typeof db.options.w != 'number' && db.options.safe == false && object.db_options.url.indexOf("safe=") == -1) {
      db.options.w = 1;
    }
...
//# sourceMappingURL=mongo_client-compiled.js.map