...
var UnorderedBulkOperation = function (collection, options) {
   ...
   */
	...
	var addToOperationsList = function (_self, docType, document) {
		...
		if (currentBatchSize + 1 >= maxWriteBatchSize || currentBatchSizeBytes + currentBatchSizeBytes >= maxBatchSizeBytes || currentBatch.batchType != docType) {
  ...
  */
	this.execute = function (_writeConcern, callback) {
		...
		if (hasWriteCommands(db.serverConfig.checkoutWriter())) return executeBatches(function (err, result) {
			callback(err, result);
		});
