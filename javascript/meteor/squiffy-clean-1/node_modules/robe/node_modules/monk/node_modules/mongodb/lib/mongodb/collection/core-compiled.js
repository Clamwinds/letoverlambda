...
var insert = function insert(docs, options, callback) {
  ...
  if (!useLegacyOps && hasWriteCommands(connection) && !Buffer.isBuffer(docs) && !(Array.isArray(docs) && docs.length > 0 && Buffer.isBuffer(docs[0]))) {
...
var insertWithWriteCommands = function (self, docs, options, callback) {
  ...
  var continueOnError = typeof options['keepGoing'] == 'boolean' ? options['keepGoing'] : false;
  continueOnError = typeof options['continueOnError'] == 'boolean' ? options['continueOnError'] : continueOnError;

  // Do we serialzie functions
  var serializeFunctions = typeof options.serializeFunctions != 'boolean' ? self.serializeFunctions : options.serializeFunctions;
  ...
  for (var index = 0, len = docs.length; index < len; ++index) {
    ...
    if (!Buffer.isBuffer(doc) && doc['_id'] == null && self.db.forceServerObjectId != true && options.forceServerObjectId != true) {
  ...
  if (docs.length == 1) {
    ...
    return self.db.command(write_command, { connection: connection,
      checkKeys: typeof options.checkKeys == 'boolean' ? options.checkKeys : true,
      serializeFunctions: serializeFunctions,
      writeCommand: true }, function (err, result) {
...
var insertAll = function insertAll(self, docs, options, callback) {
  ...
  // Pass in options
  var insertCommand = new InsertCommand(self.db, dbName + "." + self.collectionName, checkKeys, insertFlags);
...
var removeWithWriteCommands = function (self, selector, options, callback) {
  ...
  // Execute the write command
  self.db.command(write_command, { connection: connection,
    checkKeys: false,
    serializeFunctions: serializeFunctions,
    writeCommand: true }, function (err, result) {
    if (errorOptions.w == 0 && typeof callback == 'function') return callback(null, null);
...
var remove = function remove(selector, options, callback) {
  ...
  var deleteCommand = new DeleteCommand(this.db, dbName + "." + this.collectionName, selector, flags);
...
var update = function update(selector, document, options, callback) {
  ...
  var updateCommand = new UpdateCommand(this.db, dbName + "." + this.collectionName, selector, document, options);
...
var findAndModify = function findAndModify(query, sort, doc, options, callback) {
  ...
  // Execute the command
  this.db.command(queryObject, options, function (err, result) {
    if (err) return callback(err, null);
