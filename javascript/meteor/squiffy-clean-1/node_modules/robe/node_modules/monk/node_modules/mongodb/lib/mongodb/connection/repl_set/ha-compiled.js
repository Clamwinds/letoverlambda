...
HighAvailabilityProcess.prototype.start = function () {
  var self = this;
  if (this.replset._state && Object.keys(this.replset._state.addresses).length == 0) {
...
var _timeoutHandle = function (self) {
  return function () {
    ...
    if (self.server.isConnected() && !self._haProcessInProcess) {
      ...
      self.db._executeQueryCommand(DbCommand.createIsMasterCommand(self.db), { failFast: true, connection: self.server.checkoutReader() }, function (err, res) {
        // Emit ha event
        ...
        if (!master.ismaster && !master.secondary && state.addresses[master.me]) {
          self.server.close();
        ...
        if (master.primary && state.master == null || master.primary && state.master.name != master.primary) {
  ...
  if (self.replset.auth.length() > 0) {
    ...
    var connectionFunction = function (_auth, _connection, __callback) {
      ...
      for (var j = 0; j < pending; j++) {
        ...
        if (_auth.gssapiServiceName) options.gssapiServiceName = _auth.gssapiServiceName;
...
//# sourceMappingURL=ha-compiled.js.map