...
Server.prototype.inheritReplSetOptionsFrom = function (replset) {
  ...
  if (utils.isObject(replset.options.socketOptions)) {
    ...
    for (var i = 0; i < keys.length; i++) this.socketOptions[keys[i]] = replset.options.socketOptions[keys[i]];
  }
};
...
Server.prototype.connect = function (dbInstance, options, callback) {
    ...
    try {
      ...
      if (mongoReply.messageLength != message.length) {
        ...
        mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function (err) {
          ...
          if (server.recordQueryStats == true && server._state['runtimeStats'] != null && server._state.runtimeStats['queryStats'] instanceof RunningStats) {
          ...
          if ((mongoReply.responseFlag & 1 << 1) != 0 && mongoReply.documents[0].code && mongoReply.documents[0].code == 13436) {
            server.close();
          }
        });
      }
  ...
  connectionPool.on("timeout", function (err) {
    // If pool connection is already closed
    if (server._serverState === 'disconnected' || server._serverState === 'destroyed') return;
    // Set server state to disconnected
    server._serverState = 'disconnected';
    // If we have a callback return the error
    if (typeof callback === 'function') {
    ...
    if (server.isAutoReconnect() && !server.isSetMember() && server._serverState != 'destroyed' && !server._reconnectInProgreess) {
...
Server.prototype.checkoutWriter = function (read) {
  if (this._serverState == 'disconnected' || this._serverState == 'destroyed') return null;
  ...
  if (result == null && this.connectionPool != null) {
    ...
    if (connection) connection.serverCapabilities = this.serverCapabilities;
    return connection;
  } else if (result == null) {
    return null;
  } else {
