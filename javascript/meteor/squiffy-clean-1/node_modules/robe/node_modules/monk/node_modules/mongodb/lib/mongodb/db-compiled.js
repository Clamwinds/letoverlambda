...
function Db(databaseName, serverConfig, options) {
  ...
  try {
    ...
    var msg = 'Native bson parser not compiled, please compile ' + 'or avoid using native_parser=true';
    throw Error(msg);
  }
  ...
  // Set up logger
  this.logger = this.options.logger != null && typeof this.options.logger.debug == 'function' && typeof this.options.logger.error == 'function' && typeof this.options.logger.log == 'function' ? this.options.logger : { error: function (message, object) {}, log: function (message, object) {}, debug: function (message, object) {} };
  ...
  if (this.readPreference == ReadPreference.SECONDARY || this.readPreference == ReadPreference.SECONDARY_PREFERRED) {
    this.slaveOk = true;
  }
...
Db.prototype.collectionsInfo = function (collectionName, callback) {
  if (callback == null && typeof collectionName == 'function') {
    callback = collectionName;collectionName = null;
  }
  // Create selector
  var selector = {};
...
Db.prototype.authenticate = function (username, password, options, callback) {
  ...
  } else if (options.authMechanism != 'GSSAPI' && options.authMechanism != 'MONGODB-CR' && options.authMechanism != 'MONGODB-X509' && options.authMechanism != 'PLAIN') {
...
var _executeAuthCreateUserCommand = function (self, username, password, options, callback) {
  ...
  self.command(command, commandOptions, function (err, result) {
    if (err) return callback(err, null);
    callback(!result.ok ? utils.toError("Failed to add user " + username) : null, result.ok ? [{ user: username, pwd: '' }] : null);
  });
};
...
Db.prototype.command = function (selector, options, callback) {
  ...
  if (readPreference != false && ignoreCommandFilter == false) {
    if (selector['group'] || selector['aggregate'] || selector['collStats'] || selector['dbStats'] || selector['count'] || selector['distinct'] || selector['geoNear'] || selector['geoSearch'] || selector['geoWalk'] || selector['text'] || selector['cursorInfo'] || selector['parallelCollectionScan'] || selector['mapreduce'] && (selector.out == 'inline' || selector.out.inline)) {
  ...
  if ((options.writeCommand || selector['findAndModify'] || selector['insert'] || selector['update'] || selector['delete'] || selector['createUser'] || selector['updateUser'] || selector['removeUser']) && options.readPreference) {
  ...
  if (command_options.readPreference != 'primary') command_options.slaveOk = true;
  ...
  this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(execDb, selector, command_options), options, function (err, results, connection) {
    if (options.returnConnection) {
      ...
      if (results.documents[0].errmsg) return callback(utils.toError(results.documents[0]), null, connection);
      callback(null, results.documents[0], connection);
    } else {
      if (err) return callback(err, null);
      if (results.documents[0].errmsg) return callback(utils.toError(results.documents[0]), null);
...
Db.prototype.ensureIndex = function (collectionName, fieldOrSpec, options, callback) {
  ...
  if (_hasWriteConcern(writeConcern) && callback == null) throw new Error("Cannot use a writeConcern without a provided callback");
  ...
  if (typeof callback === 'function' && !onAll) {
    ...
    connection.write(db_command, function (err) {
      if (err != null) {
        ...
        if (Array.isArray(db_command)) self.serverConfig._callHandler(db_command[0].getRequestId(), null, err);else self.serverConfig._callHandler(db_command.getRequestId(), null, err);
      }
    });
...
Db.prototype._executeQueryCommand = function (db_command, options, callback) {
  ...
  if (connection && (!connection.isConnected || !connection.isConnected())) connection = null;
  ...
  if (!connection && !config.canRead(readPreference) && !config.canWrite() && config.isAutoReconnect()) {
    ...
    if (readPreference == ReadPreference.PRIMARY || readPreference == ReadPreference.PRIMARY_PREFERRED || readPreference != null && typeof readPreference == 'object' && readPreference.mode || readPreference == null) {

      // Save the command
      self.serverConfig._commandsStore.read_from_writer({ type: 'query',
      ...
      self.serverConfig._commandsStore.read({ type: 'query',
        db_command: db_command,
        options: options,
        callback: callback,
        db: self,
...
Db.prototype._executeInsertCommand = function (db_command, options, callback) {
  ...
  if (!connection && !config.canWrite() && config.isAutoReconnect()) {
    self.serverConfig._commandsStore.write({ type: 'insert',
      'db_command': db_command,
      'options': options,
      'callback': callback,
      db: self,
...
Db.connect = function (url, options, callback) {
  ...
  if (url.indexOf("safe") == -1 && url.indexOf("w") == -1 && url.indexOf("journal") == -1 && url.indexOf("j") == -1 && url.indexOf("fsync") == -1) options.w = 1;
...
var _hasWriteConcern = function (errorOptions) {
  return errorOptions == true || errorOptions.w > 0 || errorOptions.w == 'majority' || errorOptions.j == true || errorOptions.journal == true || errorOptions.fsync == true;
};
...
var _getWriteConcern = function (self, options) {
  ...
  if (finalOptions.w < 1 && (finalOptions.journal == true || finalOptions.j == true || finalOptions.fsync == true)) throw new Error("No acknowledgement using w < 1 cannot be combined with journal:true or fsync:true");
